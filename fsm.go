...
type Delegate interface {
	...
	HandleEvent(ctx context.Context, action string, fromState string, toState string, args []interface{}) error
}
...
func (m *StateMachine) Trigger(ctx context.Context, currentState string, event string, args ...interface{}) error {
	...
	var err error
	if trans.Action != "" {
		err = m.delegate.HandleEvent(ctx, trans.Action, currentState, trans.To, args)
	}
	return err
}
