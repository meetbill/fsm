...
func (p *TurnstileEventProcessor) OnActionFailure(ctx context.Context, action string, fromState string, toState string, args []interface{}, err error) {
	t := args[0].(*Turnstile)
...
func TestFSM(t *testing.T) {
	...
	//推门
	//没刷卡/投币不可进入
	err := fsm.Trigger(ctx, ts.State, "Push", ts)
	if err != nil {
	...
	//刷卡或者投币
	//不容易啊，终于解锁了
	err = fsm.Trigger(ctx, ts.State, "Coin", ts)
	if err != nil {
		t.Errorf("trigger err: %v", err)
	}
